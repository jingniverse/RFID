...
if ($files.Count -gt 0) {
    ...
    # 🌟 상위 폴더에 RFID_APP이 존재할 경우, 양쪽 연동을 위해 앱형 폴더에도 동시 업로드!
    $linkedAppPath = [System.IO.Path]::GetFullPath([System.IO.Path]::Combine($targetDir, '..', '..', 'RFID_APP', 'js', 'recipients.js'))
    if (Test-Path $linkedAppPath) {
        Copy-Item -Path $latestFile.FullName -Destination $linkedAppPath -Force
        Write-Host "   💡RFID_APP폴더에도 동시에 자동 동기화 업로드를 완료했습니다!" -ForegroundColor Green
    }
